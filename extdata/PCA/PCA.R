...
data<-read.csv("BCN.csv",row.names=1)
data<-data[,2:ncol(data)]

BM<-unlist(lapply(strsplit(colnames(data),"_"),function(x) x[2]))
HS<-unlist(lapply(strsplit(colnames(data),"_"),function(x) x[1]))
...
BMHS<-paste0(HS,"_",BM,paste="")

PCAPlot(input,BM,output="BM")
PCAPlot(input,HS,output="HS")
PCAPlot(input,BMHS,output="BM_HS")
